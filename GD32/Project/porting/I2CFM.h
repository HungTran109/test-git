...
#define RDS_ENABLE
	...
	#define	RDS_ON		1
	#define	RDS_OFF		0
	#define	RDS_INT_SEL	0	//	00 = disable the output interrupt
							//	01 = GRPRDY active as the output interrupt
							//	10 = BLK2RDY active as the output interrupt
							//	11 = BLKRDY active as the output interrupt
...
typedef union
{
    ...
    uint8_t value;
} __attribute__((packed)) decoded_rds_data_t;
#endif
